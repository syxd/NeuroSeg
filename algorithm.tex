...
\xj{we look for neurites $\tilde{\nset_{b}}\in \nset_{b}$ which overlap with ${N}_{a}$. For each neurite ${N}_{b}\in \tilde{\nset_{b}}$, if the overlap volume between ${N}_{b}$ and ${N}_{a}$ is larger than a threshold $\delta_{ovlp}$, we fuse these two neurites together to get a more complete neuron.}
