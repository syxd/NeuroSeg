...
\subsubsection{Neurite Fusion from Adjacent Blocks}
\label{sec:fusion}
...
\delete{However, directly assembling matched neurites would cause over-tracing and topological discrepancy, as shown in Fig.~\ref{fig:overlap_discrepancy}.
...
%\xj{compartment->dots. how do you get these point from branches? uniformly sampling? }
...
%we remove the neuronal points in branch $\mathcal{H_A}$ and branch $\mathcal{H_B}$ that near the boundary of the corresponding block, and all branches that are connected to the removed points will also be removed, as shown in Fig~\ref{fig:fusion_algorithm} (e). Here, this distance is set to be $70$ voxels.\xj{What distance? distance between what? how do you compute the distance?}
...
%The searching radius is empirically set to be $10$ voxels.
