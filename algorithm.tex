...
Therefore, we first divide the input image into blocks that are averagely in size of $0.5mm\times 1mm\times \textbf{0.25}mm$ with some \textbf{overlaps}. 
%
For each block, we first enhance noisy image signals by deep neural network, which is trained by progressive learning from reconstructed neurons using traditional tracing algorithms. Then we reconstruct neurons in each block using the enhanced image. 
%
...
\begin{figure}[t]
	...
	\caption{Comparison of neuronal population reconstruction on an enhanced image block. (a) Using NGPST without using neurite tips as pseudo somas. (b) Using NGPST with neurite tips as pseudo somas. Separated neurons are shown in different colors. 
		\xj{What about two neurites belong to the same neuron but separate at the boundary?} }
...
If there still remains a branch  $\brc_b$ which is not connected to the neurite $N_{ab}$, $N_{ab}$ will be updated by assembling the remaining branch.
