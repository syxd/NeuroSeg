...
%
Finally, the overlapped neurites reconstructed from local blocks are fused to get a complete reconstruction of neurons. 
...
% The neuronal compartments which are close to the block boundary typically indicate the continuity of the structure of neurons. We call these compartments as terminal tips and use them as the potential continuous signals for searching the next blocks to grow the neuronal structure from each initial block. %all boundary tips from the previous tile’s reconstruction were detected as the reference locations
...
\xj{Each branch is sampled uniformly into a set of fragments}, as Fig.~\ref{fig:fusion_algorithm} (d) shows.
%
...
The same deletion operation is performed \xj{on the branch $F_b$ in $N_b$} that are located in the boundary regions of block $\blk_b$.
