...
Despite their great \md{improvements} in this task, several limitations still remain.
%
One main bottleneck is that the base tracers used for neuron tracing in image blocks typically employ a series of traditional image processing algorithms \md{such as binarization, fast marching, fusion,} etc. 
...
%
For dense neuronal population reconstruction in ultra-scale OM images, dense neurites may cross with each other, making the neuron tracing much more challenging in low-contrast and noisy OM images. 
% 
Following the commonly used block-by-block framework, we introduce UltraNPR, which utilizes our PLNPR approach as the base tracer \md{in} blocks, and design a novel tracing and fusion strategy for dense neuronal populations.
%
...
In summary, we propose PLNPR, a progressive learning framework that integrates traditional neuron tracing methods and deep segmentation networks for neuron population reconstruction without using manual annotations.
%
We integrate our PLNPR with a block-wise propagation and fusion strategy which can reconstruct dense neuronal populations from trillions of voxels in ultra-scale OM images. 
%
