...
Although NGPST~\cite{Quan2015} achieves better performance of neuronal population reconstruction \xj{what do you mean by neuronal identity? identification? separation?}, it still remains difficult to extract subtle neuron voxels for NGPST by using hand-crafted features.
%
...
\xj{Do you use 160x160x160 on the Visor-40 dataset? Why different? no data rotation for augmentation? how about data balance? }
...
%
Comparing with \cite{Li2017}, our PLNPR achieves higher PDS and ESA, and comparable performance in DSA metric with the best one.
\xj{Discuss more about these metrics? why lower DSA?}
%
In particular, unlike~\cite{Li2017} which requires on a strongly supervised network, our method obtains even better performance without any manual annotations.
...
\xj{check the consistency with the physical size of blocks in Sec. 3.}
