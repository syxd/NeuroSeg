...
\end{minipage}}
	\caption{ Performance comparison with seven neuron tracing methods on the raw VISoR-40 test images and on the same images enhanced by our PLNPR.
...
To prove the effectiveness of our method on neuronal population reconstruction, we compare it with seven widely used neuron tracing methods, including APP1~\cite{Peng2011},  APP2~\cite{Xiao2013}, MOST~\cite{Wu2014}, SmartTracing (ST)~\cite{Chen2015}, NGPST~\cite{Quan2015}, TReMAP~\cite{Zhou2016} and  FMST~\cite{Yang2019}.
...
Fig.~\ref{fig:compare_BigNeuron} visualizes the reconstructed neurons in two test images using different tracing methods.
``PLNPR-NGPST" means that we progressively train the DSN model using pseudo labels generated by NGPST~\cite{Quan2015}.
...
%have tuned the parameters for more than 10 times for each method and each test image block to find the best results.
...
%\subsubsection{Experimental Settings}
To reconstruct the dense neuronal population from an ultra-scale image, we divide the entire image into blocks in size of $1120\times 2048\times 869$ considering the memory and computational efficiency of our PLNPR.
%
...
We also compare our method with NGPST~\cite{Quan2015}, which is the state-of-the-art method for neuronal population reconstruction.
...
Fig.~\ref{fig:reconstruct_blocks} compares the neuronal population reconstruction results of different methods.
%
